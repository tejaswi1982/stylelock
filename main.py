"""
StyleLock AI - Backend Server v52
=================================
FIXES from V51:
...
app = FastAPI(title="StyleLock AI", version="52")
...
async def create_razorpay_order():
    ...
    try:
        ...
        order = client.order.create({"amount": 7900, "currency": "INR", "payment_capture": 1})
        return {"order_id": order["id"], "amount": 7900, "currency": "INR", "key_id": RAZORPAY_KEY_ID}
    except Exception as e:
...
async def generate_looks(request: Request):
    ...
    try:
        ...
        for rec in recs:
            looks.append({
                "name": rec["tier"],  # BOLD, CLEAN, TRENDING
                "full_name": rec["name"],
                "image": rec.get("preview_url") or f"/static/images/hairstyle_{rec['tier'].lower()}.jpg",
                "match_percentage": rec["match_percentage"],
                "achievability": rec["achievability"],
                "vibe": rec.get("vibe", ""),
...
if __name__ == "__main__":
    ...
    print(f"🚀 StyleLock AI v52 starting on port {port}")
