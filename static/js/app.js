...
function getTierNote(tier) {
    if (tier === 'BOLD') return 'go loud';
    if (tier === 'CLEAN') return 'stay sharp';
    return 'right now';
}
        ...
        .map((look, idx) => {
            ...
            return `
                <div class="results-slide" data-idx="${idx}">
                    <article class="slide-card ${tierClass(look.tier)}">
...
function selectLook(idx) {
    ...
    byId('selectLookBtn').onclick = () => lockLook(idx);
    show('cutcardScreen');
}
...
function bindEvents() {
    ...
    byId('takePhotoBtn').addEventListener('click', openActionSheet);
